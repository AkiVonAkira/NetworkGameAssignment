    ...
    public class XtremeFPSEditor : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (enablePlayerSetupPanel)
            {
                ...
                virtualCamera = (CinemachineCamera)EditorGUILayout.ObjectField(new GUIContent("Virtual Camera", "The referrence to the virtual camera (Leave empty if none exists already)."), virtualCamera, typeof(CinemachineCamera), true);
        ...
        private void CreateParentObjectAndOtherComponents()
        {
            ...
            if (virtualCamera == null ||
                !virtualCamera.TryGetComponent<CinemachineCamera>(out _))
            {
                ...
                virtualCamera = playerVirtualCamera.GetComponent<CinemachineCamera>();
            }
        ...
        private void DefaultPlayerValues()
        {
            ...
            {
                virtualCamera.GetOrAddComponent<CinemachineHardLockToTarget>();
            }

            CinemachineComponentBase aim = virtualCamera.GetCinemachineComponent(CinemachineCore.Stage.Aim);
            if (aim is not CinemachineRotateWithFollowTarget)
            {
                virtualCamera.GetOrAddComponent<CinemachineRotateWithFollowTarget>();
            }
